...
(** [init_players] is a list of players with no tiles on hand. *)
let init_players id = 
  let init_this_player this_id = 
    Player.init_player this_id false false [] [] [] in 
  init id [] init_this_player
...
let init_state n_of_t n_of_p = 
  {
    wall_tiles = init_tiles n_of_t;
    players = init_players n_of_p
  }

(* [shuffle_list] is the shuffled list of [a]. *)
